...
so_nhị_phan_list = chuoi_so_nhi_phan.split(',')
so_chia_het_cho_5 = [so for so in so_nhị_phan_list if chia_het_cho_5(so)]
#in ra các số nhị phân chia hết cho 5
if len(so_chia_het_cho_5)>0:
    ket_qua = ','.join(so_chia_het_cho_5)

    print("các số nhị phân chia kết cho 5 là:", ket_qua)
else:
    print("Không có số nhị phân nào chia hết cho 5 trong chuỗi nhập.")